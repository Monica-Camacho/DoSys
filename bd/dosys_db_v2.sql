-- =================================================================
-- Script Unificado y Corregido para la Base de Datos dosys_db_v2
-- Versión: 1.0
-- Descripción: Este script crea la estructura completa y normalizada de la base de datos
-- e inserta todos los datos de respaldo. Se han eliminado las duplicidades y
-- corregido los errores lógicos y de referencias.
-- =================================================================
...
--
CREATE DATABASE IF NOT EXISTS `dosys_db_v2` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE `dosys_db_v2`;
...
--
-- Tabla: `representantes` (Aunque no se usa en la nueva estructura, se mantiene por si hay datos heredados)
--
...
--
-- Tabla: `empresas_perfil` (Estructura Normalizada)
--
...
--
-- Tabla: `organizaciones_perfil` (Estructura Normalizada)
--
...
--
-- Tabla de unión: `usuarios_x_empresas`
--
CREATE TABLE `usuarios_x_empresas` (
  `usuario_id` int(11) NOT NULL,
  `empresa_id` int(11) NOT NULL,
  `rol_en_entidad` enum('Administrador','Gestor','Miembro') NOT NULL DEFAULT 'Miembro',
  `fecha_asignacion` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Tabla de unión: `usuarios_x_organizaciones`
--
CREATE TABLE `usuarios_x_organizaciones` (
  `usuario_id` int(11) NOT NULL,
  `organizacion_id` int(11) NOT NULL,
  `rol_en_entidad` enum('Administrador','Gestor de Casos','Voluntario') NOT NULL DEFAULT 'Voluntario',
  `fecha_asignacion` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
--
-- Tabla: `empresas_apoyos` (Con FK corregida)
--
...
--
-- Tabla: `solicitudes_dispositivos`
--
...
(3, 'Asma'), (2, 'Diabetes Tipo 2'), (1, 'Hipertensión'), (4, 'Obesidad');

INSERT INTO `estatus_aviso` (`id`, `nombre`) VALUES
(1, 'Pendiente de Validar'), (2, 'Activo'), (3, 'Completado'), (4, 'Rechazado');

INSERT INTO `estatus_donacion` (`id`, `nombre`) VALUES
(1, 'Pendiente de Aprobación'), (2, 'Aprobado'), (3, 'Recibido'), (4, 'No Concretado');

INSERT INTO `categorias_donacion` (`id`, `nombre`) VALUES
(1, 'Sangre'), (2, 'Medicamentos'), (3, 'Dispositivos'), (4, 'Otro');

INSERT INTO `urgencia_niveles` (`id`, `nombre`, `color_hex`) VALUES
(1, 'Baja', '#28a745'), (2, 'Media', '#ffc107'), (3, 'Alta', '#fd7e14'), (4, 'Crítica', '#dc3545');

INSERT INTO `roles` (`id`, `nombre`) VALUES
(1, 'Administrador'), (2, 'Creador'), (3, 'Visualizador'), (4, 'Gestor de Casos');

INSERT INTO `tipos_usuario` (`id`, `nombre`) VALUES
(1, 'Persona'), (2, 'Empresa'), (3, 'Organizacion'), (4, 'SuperAdmin');
...
(1, 'Descuentos y Beneficios', 1), (2, 'Difusión y Comunicación', 0),
(3, 'Apoyo Logístico', 0), (4, 'Servicios Gratuitos', 1);
...
-- Inserción de datos de empresas y creación de la relación con usuarios
-- NOTA: Se usa el `usuario_id` del volcado original como el nuevo `id` de la empresa para mantener la consistencia.
INSERT INTO `empresas_perfil` (`id`, `nombre_comercial`, `razon_social`, `rfc`, `telefono_empresa`, `descripcion`, `direccion_comercial_id`, `empresa_padre_id`) VALUES
(2, 'Farmacias del Ahorro', 'Farmacias del Ahorro SA de CV', 'FAH850101XXX', '9933109876', 'Farmacia líder en el sureste.', 2, NULL);
INSERT INTO `usuarios_x_empresas` (`usuario_id`, `empresa_id`, `rol_en_entidad`) VALUES (2, 2, 'Administrador');

-- Inserción de datos de organizaciones y creación de la relación con usuarios
INSERT INTO `organizaciones_perfil` (`id`, `nombre_organizacion`, `cluni`, `logo_documento_id`, `direccion_id`, `mision`, `organizacion_padre_id`) VALUES
(3, 'Cruz Verde Tabasco', 'CVT123456789', NULL, 3, 'Proveer ayuda médica y humanitaria a la comunidad de Tabasco.', NULL),
(13, 'org1', 'org1', NULL, NULL, NULL, NULL);
INSERT INTO `usuarios_x_organizaciones` (`usuario_id`, `organizacion_id`, `rol_en_entidad`) VALUES
(3, 3, 'Administrador'),
(13, 13, 'Administrador');
...
ALTER TABLE `alergias` ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `nombre_UNIQUE` (`nombre`);
ALTER TABLE `avisos` ADD PRIMARY KEY (`id`), ADD KEY `fk_avisos_usuarios_creador` (`creador_id`), ADD KEY `fk_avisos_usuarios_organizacion` (`organizacion_id`), ADD KEY `fk_avisos_beneficiarios` (`beneficiario_id`), ADD KEY `fk_avisos_categorias_donacion` (`categoria_id`), ADD KEY `fk_avisos_urgencia_niveles` (`urgencia_id`), ADD KEY `fk_avisos_estatus_aviso` (`estatus_id`), ADD KEY `fk_avisos_usuarios_responsable` (`contacto_responsable_id`);
ALTER TABLE `avisos_documentos` ADD PRIMARY KEY (`id`), ADD KEY `fk_avisos_documentos_avisos` (`aviso_id`);
ALTER TABLE `beneficiarios` ADD PRIMARY KEY (`id`);
ALTER TABLE `categorias_donacion` ADD PRIMARY KEY (`id`);
ALTER TABLE `direcciones` ADD PRIMARY KEY (`id`);
ALTER TABLE `documentos` ADD PRIMARY KEY (`id`), ADD KEY `fk_documentos_usuarios` (`propietario_id`), ADD KEY `fk_documentos_tipos` (`tipo_documento_id`);
ALTER TABLE `donaciones` ADD PRIMARY KEY (`id`), ADD KEY `fk_donaciones_avisos` (`aviso_id`), ADD KEY `fk_donaciones_usuarios` (`donante_id`), ADD KEY `fk_donaciones_estatus_donacion` (`estatus_id`);
ALTER TABLE `donantes_beneficios` ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `codigo_canje_UNIQUE` (`codigo_canje`), ADD KEY `fk_db_donaciones` (`donacion_id`), ADD KEY `fk_db_usuarios` (`usuario_id`), ADD KEY `fk_db_empresas_apoyos` (`apoyo_id`);
ALTER TABLE `empresas_apoyos` ADD PRIMARY KEY (`id`), ADD KEY `fk_ea_usuarios` (`empresa_id`), ADD KEY `fk_ea_tipos_apoyo` (`tipo_apoyo_id`);
ALTER TABLE `empresas_perfil` ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `rfc_UNIQUE` (`rfc`), ADD KEY `fk_ep_dir_comercial` (`direccion_comercial_id`), ADD KEY `fk_ep_logo` (`logo_documento_id`), ADD KEY `fk_ep_padre` (`empresa_padre_id`);
ALTER TABLE `enfermedades` ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `nombre_UNIQUE` (`nombre`);
ALTER TABLE `estatus_aviso` ADD PRIMARY KEY (`id`);
ALTER TABLE `estatus_donacion` ADD PRIMARY KEY (`id`);
ALTER TABLE `organizaciones_perfil` ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `cluni_UNIQUE` (`cluni`), ADD KEY `fk_op_direccion` (`direccion_id`), ADD KEY `fk_op_logo` (`logo_documento_id`), ADD KEY `fk_op_padre` (`organizacion_padre_id`);
ALTER TABLE `personas_perfil` ADD PRIMARY KEY (`usuario_id`), ADD KEY `fk_pp_tipos_sangre` (`tipo_sangre_id`), ADD KEY `fk_pp_direcciones` (`direccion_id`);
ALTER TABLE `personas_x_alergias` ADD PRIMARY KEY (`usuario_id`,`alergia_id`), ADD KEY `fk_pxa_alergias` (`alergia_id`);
ALTER TABLE `personas_x_enfermedades` ADD PRIMARY KEY (`usuario_id`,`enfermedad_id`), ADD KEY `fk_pxe_enfermedades` (`enfermedad_id`);
ALTER TABLE `puntos_donacion` ADD PRIMARY KEY (`id`), ADD KEY `fk_pd_tipos` (`tipo_id`), ADD KEY `fk_pd_usuarios` (`organizacion_responsable_id`), ADD KEY `fk_pd_direcciones` (`direccion_id`);
ALTER TABLE `representantes` ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `email_UNIQUE` (`email`);
ALTER TABLE `roles` ADD PRIMARY KEY (`id`);
ALTER TABLE `solicitudes_dispositivos` ADD PRIMARY KEY (`id`), ADD KEY `fk_sd_avisos` (`aviso_id`);
ALTER TABLE `solicitudes_medicamentos` ADD PRIMARY KEY (`id`), ADD KEY `fk_sm_avisos` (`aviso_id`);
ALTER TABLE `solicitudes_sangre` ADD PRIMARY KEY (`aviso_id`), ADD KEY `fk_ss_tipos_sangre` (`tipo_sangre_id`);
ALTER TABLE `tipos_apoyo` ADD PRIMARY KEY (`id`);
ALTER TABLE `tipos_documento` ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `codigo_UNIQUE` (`codigo`);
ALTER TABLE `tipos_punto_donacion` ADD PRIMARY KEY (`id`);
ALTER TABLE `tipos_sangre` ADD PRIMARY KEY (`id`);
ALTER TABLE `tipos_usuario` ADD PRIMARY KEY (`id`);
ALTER TABLE `urgencia_niveles` ADD PRIMARY KEY (`id`);
ALTER TABLE `usuarios` ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `email_UNIQUE` (`email`), ADD KEY `fk_u_tipos_usuario` (`tipo_usuario_id`), ADD KEY `fk_u_roles` (`rol_id`);
ALTER TABLE `usuarios_x_empresas` ADD PRIMARY KEY (`usuario_id`, `empresa_id`), ADD KEY `fk_uxe_empresa` (`empresa_id`);
ALTER TABLE `usuarios_x_organizaciones` ADD PRIMARY KEY (`usuario_id`, `organizacion_id`), ADD KEY `fk_uxo_organizacion` (`organizacion_id`);

-- --------------------------------------------------------
-- AUTO_INCREMENT
-- --------------------------------------------------------

ALTER TABLE `alergias` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
ALTER TABLE `avisos` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
ALTER TABLE `avisos_documentos` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
ALTER TABLE `beneficiarios` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
ALTER TABLE `categorias_donacion` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
ALTER TABLE `direcciones` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;
ALTER TABLE `documentos` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
ALTER TABLE `donaciones` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
ALTER TABLE `donantes_beneficios` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
ALTER TABLE `empresas_apoyos` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
ALTER TABLE `empresas_perfil` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
ALTER TABLE `enfermedades` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
ALTER TABLE `estatus_aviso` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
ALTER TABLE `estatus_donacion` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
ALTER TABLE `organizaciones_perfil` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;
ALTER TABLE `puntos_donacion` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
ALTER TABLE `representantes` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;
ALTER TABLE `roles` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
ALTER TABLE `solicitudes_dispositivos` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
ALTER TABLE `solicitudes_medicamentos` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
ALTER TABLE `tipos_apoyo` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
ALTER TABLE `tipos_documento` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
ALTER TABLE `tipos_punto_donacion` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
ALTER TABLE `tipos_sangre` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
ALTER TABLE `tipos_usuario` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
ALTER TABLE `urgencia_niveles` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
...
SET FOREIGN_KEY_CHECKS=1; -- Reactivamos la revisión.
COMMIT;
