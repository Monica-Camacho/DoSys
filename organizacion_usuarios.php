...
// 1. AUTENTICACIÓN Y OBTENCIÓN DE ID DE ORGANIZACIÓN Y ROL
...
// Se busca la organización y el rol del usuario que ha iniciado sesión
...
if ($resultado_permisos->num_rows === 0) {
    header('Location: persona_dashboard.php'); // No pertenece a una organización
    exit();
}
...
// 3. FUNCIÓN PARA OBTENER VOLUNTARIOS CON FILTROS
function obtener_voluntarios($conexion, $organizacion_id, $busqueda, $rol_id, $estado) {
    $sql = "SELECT 
                u.id AS usuario_id, u.email, u.estado, u.rol_id,
                pp.nombre, pp.apellido_paterno,
                r.nombre AS rol_nombre
            FROM usuarios_x_organizaciones uxo
            JOIN usuarios u ON uxo.usuario_id = u.id
            JOIN personas_perfil pp ON u.id = pp.usuario_id
            LEFT JOIN roles r ON u.rol_id = r.id
            WHERE uxo.organizacion_id = ?";
...
$voluntarios = obtener_voluntarios($conexion, $organizacion_id, $filtro_busqueda, $filtro_rol, $filtro_estado);
        ...
        <div class="container">
            <div class="card border-0 shadow-sm"><div class="card-body p-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <?php if ($rol_usuario_logueado == 1): // Solo el admin (rol 1) puede añadir ?>
                        <a href="organizacion_usuario_crear.php" class="btn btn-primary"><i class="fas fa-user-plus me-2"></i>Añadir Nuevo Voluntario</a>
                    <?php endif; ?>
                </div>
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            <?php if (empty($voluntarios)): ?>
                                <tr><td colspan="5" class="text-center text-muted">No se encontraron voluntarios con los filtros seleccionados.</td></tr>
                            <?php else: foreach ($voluntarios as $voluntario): ?>
                            <tr>
                                <td><strong><?php echo htmlspecialchars($voluntario['nombre'] . ' ' . $voluntario['apellido_paterno']); ?></strong></td>
                                ...
                                <td class="text-center">
                                    <?php
                                        $mapa_colores_rol = [
                                            ...
                                            'Visualizador' => 'bg-info'
                                        ];
                                        $clase_rol = $mapa_colores_rol[$voluntario['rol_nombre']] ?? 'bg-secondary';
                                    ?>
                                    <span class="badge <?php echo $clase_rol; ?>">
                                        <?php echo htmlspecialchars($voluntario['rol_nombre'] ?? 'Sin rol'); ?>
                                    </span>
                                </td>
                                <td class="text-center">
                                    <?php
                                        ...
                                        $clase_estado = $mapa_colores_estado[$voluntario['estado']] ?? 'bg-light text-dark';
                                    ?>
                                    <span class="badge <?php echo $clase_estado; ?>">
                                        <?php echo htmlspecialchars($voluntario['estado']); ?>
                                    </span>
                                </td>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="editarUsuarioModalLabel">Editar Voluntario</h5>
