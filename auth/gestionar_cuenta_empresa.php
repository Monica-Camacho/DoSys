...
if (empty($accion) || empty($password) || !in_array($accion, ['desactivar', 'reactivar'])) {
    ...
    header('Location: ../empresa_configuracion.php');
    exit();
}

// 2. OBTENER DATOS Y PERMISOS DEL ADMINISTRADOR
$sql_admin = "SELECT u.password_hash, u.rol_id, uxe.empresa_id 
              FROM usuarios u 
...
if (!$admin || $admin['rol_id'] != 1) {
    ...
    header('Location: ../empresas_configuracion.php');
    exit();
}
...
$mensaje_exito = ($accion === 'desactivar') ? 'Tu empresa ha sido desactivada.' : 'Tu empresa ha sido reactivada con éxito.';

$sql_update = "UPDATE empresas_perfil SET estado = ? WHERE id = ?";
$stmt_update = $conexion->prepare($sql_update);
$stmt_update->bind_param("si", $nuevo_estado, $empresa_id);
...
if ($stmt_update->execute()) {
    ...
    $_SESSION['error_message'] = "Ocurrió un error al actualizar el estado de la empresa.";
}
